...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES['photo'])) {
    ...
    $errorMessage = "";  
    // Sprawdzanie, czy plik o tej samej nazwie już istnieje w katalogu
    if (file_exists($uploaded_file)) {
        $errorMessage = "To zdjęcie już istnieje.";
    } else {
        if (move_uploaded_file($_FILES["photo"]["tmp_name"], $uploaded_file)) {
            $sql = "INSERT INTO server_content (server_id, image_path, note, signature) VALUES ('$server_id', '$uploaded_file', '$note', '$signature')";
            if (mysqli_query($connection, $sql)) {
                echo "<script type='text/javascript'>window.location.href = 'view.php?id=" . $server_id . "';</script>";
                exit();
            } else {
    ...
    <?php if ($isServerActive): ?>
        ...
        <form action="" method="post" enctype="multipart/form-data" onsubmit="return submitForm(this);">
            ...
            <div class="file-upload">
                <label for="file-upload">Wybierz plik</label>
                <input type="file" name="photo" id="file-upload" accept="image/*" required onchange="displayFileName(this)">

            </div>
            <span id="file-name" class="file-name"></span>
            <br>
            <label>Wpis/Życzenia:</label>
            <textarea name="note"></textarea>
            <label>Podpis:</label>
            <input type="text" name="signature">
            
            <span style="color: red; display: block; margin: 1rem;"><?php echo $errorMessage; ?></span>

            <input type="submit" value="Dodaj" id="submitButton">
...
function submitForm(form) {
    ...
    if (fileInput.files.length > 0) {
        submitButton.value = "Dodawanie...";

        setTimeout(function() {
            form.submit(); // Ręczne wysłanie formularza po krótkim opóźnieniu
        }, 300); // Opóźnienie 300ms. Możesz dostosować tę wartość w zależności od potrzeb

        return false; // Zapobieganie domyślnemu wysłaniu formularza
    }

    return true; // Jeśli nie ma pliku, pozwól formularzowi na normalne wysłanie
}
